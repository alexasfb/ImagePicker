...
open class TopView: UIView {
  ...
  open lazy var flashButton: UIButton = { [unowned self] in
    let button = UIButton()
    button.setImage(AssetManager.getImage("AUTO"), for: [])
    button.setTitle("AUTO", for: [])
  ...
  open lazy var rotateCamera: UIButton = { [unowned self] in
    ...
    button.setImage(AssetManager.getImage("cameraIcon"), for: [])
  ...
  @objc func flashButtonDidPress(_ button: UIButton) {
    ...
    case 1:
      button.setTitleColor(UIColor(red: 0.98, green: 0.98, blue: 0.45, alpha: 1), for: [])
    ...
    button.setImage(AssetManager.getImage(newTitle), for: [])
    button.setTitle(newTitle, for: [])
